...
namespace ZJZYDYDX_JTZDBZLZJZLFJYLPB
{
	...
	public partial class GamePanel : UIPanel
	{
		...
		[SerializeField] List<DrugItem> drugInstances = new List<DrugItem>();
		...
		public void OpenDrugStorageAnimation(Case nowCase, DrugInfos datas)
		{
			NowCase = nowCase;
			riDrugStorage.gameObject.SetActive(true);

			for (int i = 0; i < datas.drugInfos.Count; i++)
			{
				DrugItem drugItem = drugInstances[i];
				DrugInfo data = datas.drugInfos[i];

				drugItem.gameObject.SetActive(true);
				//TODO:
				drugItem.img.sprite = data.sprite;
				drugItem.tmp.text = data.drugName.ToString();
			}

		}

		public void CloseDrugStorageAnimation()
		{
			riDrugStorage.gameObject.SetActive(false);
		}

		public async UniTask<float> WaitQuestion(string questionPath)
		{
			...
			return svQuestion.GetedTotalScore;
		}
		...
		public async UniTask WaitProgressBar(string strContent, float waitTime)
		{
			tmpProgressBar.text = strContent;
			imgFullBar.fillAmount = 0;
			objProgressBar.gameObject.SetActive(true);
			await DOTween.To(() => imgFullBar.fillAmount, value => imgFullBar.fillAmount = value, 1, waitTime).AsyncWaitForCompletion();
