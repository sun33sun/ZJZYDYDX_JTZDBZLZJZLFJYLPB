	...
	public partial class Task_MaleStudent : ViewController
	{
		...
		public void SecondCokeClock()
		{
			StartAsync(async () =>
			{
				...
				ReportPanelData.Instance.RecordStartTime(ReportName.病例一_二诊);
			});
		}

		public void FeedingMedicine()
		{
			StartAsync(() => _gamePanel.WaitProgressBar("患者服药中", 10));
		}
		...
		public void EndTip()
		{
			UniTask.Void(async () =>
			{
				...
				"症状：手足已温，神清汗收，阳复脉出，血压100-70mmHg（毫米汞柱）。觉气短口微渴。投生脉散三剂而愈。" +
				"\n\n医嘱：温疟伤阴，吐泻竭液，阴亡阳脱，故首先回阳救脱，投四逆加人参汤；阳问脉复，继用生脉散，益气养阴以善后。",
				"结束二诊");
				//实验报告_二诊
				ReportPanelData.Instance.RecordGetedScore(ReportName.病例一_二诊);
				ReportPanelData.Instance.RecordEndTime(ReportName.病例一_二诊);
