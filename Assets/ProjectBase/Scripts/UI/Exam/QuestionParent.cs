...
namespace ProjectBase.Exam
{
	public class QuestionParent : MonoBehaviour
	{
		...
		public float GetedTotalScore => _scoreList.Sum();
		...
		public void LoadQuestion(string jsonPath, string questionPrefabPath, string optionPrefabPath)
		{
			...
			for (int i = 0; i < _questionDatas.Count; i++)
			{
				...
				_items[i].SetQuestion($"{i + 1}、" + _questionDatas[i].strHead, AllChoiceContent, _questionDatas[i].choiceType,
					optionPrefab);

				//错误检测
				if (_questionDatas[i].choiceType == QuestionData.ChoiceType.Single)
				{
					if (_questionDatas[i].multipleRights.Count > 0)
					{
						print($"单选错误：{_questionDatas[i].strHead}，有多选正确值{_questionDatas[i].multipleRights}；单选正确值为：{_questionDatas[i].singleRight}");
					}
				}
                else
                {
					if (_questionDatas[i].multipleRights.Count < 1)
					{
						print($"多选错误：{_questionDatas[i].strHead}，有单选正确值{_questionDatas[i].singleRight}；多选正确值为：{_questionDatas[i].multipleRights}");
					}
				}
            }
